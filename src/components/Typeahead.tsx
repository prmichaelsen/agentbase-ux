...
export function Typeahead({
  ...
  const selectedOptionDetails = selectedOptions.map((id) => {
    ...
    return { id, label: option ? option.label : 'Unknown Option' }
  })

  if (isLoading) {
    return (
      <div className="space-y-2">
        <div className="h-10 w-full animate-pulse rounded bg-bg-elevated" />
    ...
    <div className={`space-y-1 ${className}`} ref={comboboxRef}>
                ...
                {renderOption ? (
                  ...
                  <div className="flex items-center">
                    <input
                      ...
                      readOnly
                      className="mr-2 h-4 w-4 rounded border-border-default text-brand-primary focus:ring-brand-primary"
                    />
      ...
      <div className="flex flex-wrap gap-1.5">
        {selectedOptionDetails.map((option) => (
          <div
            key={option.id}
            className="flex items-center rounded-full bg-brand-primary/10 px-2 py-0.5 text-xs text-brand-primary"
          >
